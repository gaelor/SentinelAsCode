...
git clone https://github.com/Neo23x0/sigma.git tmp/Sigma
$Date = Get-Date -Format "ddMMyyyy"
$Files = Get-ChildItem -Path '.\tmp\Sigma\'  -Filter *.yaml -Recurse -File -Name | Select-String -Pattern "rules"
...
foreach ($file in $Files){
    if ($file -ne $Files[-1]) {
        ...
        $HuntingRulesTemplate += "    {`r`n"
        $HuntingRulesTemplate += "      `"author`": `"sigma`",`r`n"
        $HuntingRulesTemplate += "      `"displayName`": `"" + $huntingrules.name + "`",`r`n"
        $HuntingRulesTemplate += "      `"reference`": `"https://raw.githubusercontent.com/Neo23x0/sigma/master/rules/network/" + $file.tostring().replace("\", "/").replace(" ","%20") + "`",`r`n"
...
$HuntingRulesTemplate += "  ]`r`n"
$HuntingRulesTemplate += "}`r`n"
Out-File -Path $HuntingRulesFolder\"MS_hunting-rules_"$Date".json" -InputObject $HuntingRulesTemplate
Start-Sleep -s 20

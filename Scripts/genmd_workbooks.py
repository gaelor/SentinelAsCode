...
for key,workbook in myjson.items():
    if key == "parameters":
        print('## ' + filename)
        print('### Workbook Tags\n')
        print('> Author: ['+ workbook["author"]["defaultValue"] + '](https://www.metsys.fr/)' + '\n')
        print('> Reference: [Link to medium post](https://github.com/Azure/Azure-Sentinel/tree/master/' + sys.argv[1] + ')\n')
        print('### Workbook details\n')
        print('> Description: ' + workbook["description"]["defaultValue"] + '\n')
    if key == "resources":
        print('> Query:\n')
        print('```t')
        print(workbook[0]["properties"]["serializedData"])
        print('```\n')